...
-- ----------------------------------------------------------------------------
-- DELIVERY TRACKING + CHALLAN — additive columns on the existing sales table.
-- Safe to re-run: uses "add column if not exists".
-- ----------------------------------------------------------------------------
alter table sales add column if not exists challan_no       text;
alter table sales add column if not exists delivery_status  text not null default 'pending' check (delivery_status in ('pending','dispatched','delivered'));
alter table sales add column if not exists delivery_date    date;
alter table sales add column if not exists transport_note   text;
alter table sales add column if not exists received_by      text;

create unique index if not exists idx_sales_challan_no on sales (challan_no);
